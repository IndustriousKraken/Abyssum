...
- **Scanner selection:** the orchestrator's scanner registry resolves stable scanner ids
  (`rest_discovery`, `openapi_discovery`, `cors`, `bac`, `idor`, `graphql`). Invalid ids
  are rejected by `clap`/the registry before any work starts.
- **Output rendering:**
...
Success → 0. Bad input (unknown scanner, unparseable target), a scan that fails, or a
user interrupt → distinct non-zero codes, so CI can branch on outcome.
