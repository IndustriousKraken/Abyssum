...
- [ ] 1.2 Validate `--scanners` against the orchestrator registry's `available()` ids (the source of truth, not a duplicated hardcoded enum); restrict `--output` to `table`/`json`/`csv`
