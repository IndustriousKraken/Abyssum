...
- [ ] 2.2 Build the chat request as a fixed system message (framing authorized-testing analysis) plus a user message carrying the finding's scanner id, target, status, severity, title, and evidence
...
## 5. Analysis surface
- [ ] 5.1 Add an "Analyze with AI" action to the web finding-detail view that calls `analyze_finding` for that finding and renders the returned analysis, showing a clear notice in place on a non-fatal error

## 6. Tests (local only — no real providers or targets)
- [ ] 6.1 Unit-test prompt assembly from a representative finding, including the authorized-context system message and evidence truncation
- [ ] 6.2 Integration-test against a local mock OpenAI-compatible server: happy path returns the analysis text
- [ ] 6.3 Test that with NO key configured the request succeeds and the mock observed no authorization header
- [ ] 6.4 Test that with a key configured the request carries a bearer credential
- [ ] 6.5 Test that a 500 response, a malformed body, and a timeout each surface a clear non-fatal error and do not abort the caller
- [ ] 6.6 Test that an analyze request with AI disabled returns a clear notice and changes nothing else
