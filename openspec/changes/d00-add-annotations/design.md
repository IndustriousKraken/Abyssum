...
```
notes        — freeform text, author, created_at, edited_at;
               session_id -> sessions.session_id, optional finding_id -> findings.finding_id
tags         — unique normalized name, hex color, optional description
session_tags — many-to-many join between sessions and tags
```

A note belongs to exactly one session (referenced by the public `session_id`); it may
additionally reference one finding within that session by the finding's stable public
`finding_id` (the identifier `add-result-persistence` assigns each stored finding) for a
finding-level note, or none for a session-level note. Tags attach only to sessions (matching
v1; finding-level tagging is not in scope).
...
neutral gray `#6b7280`) and then applies it, so an operator can tag fluidly without a separate
create step — while an explicit create path still exists to set a specific color/description
up front.
...
| Note target | must reference an existing session by `session_id` (and, for a finding-note, an existing `finding_id` belonging to that session) |
