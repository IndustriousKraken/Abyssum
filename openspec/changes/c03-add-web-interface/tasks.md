...
- [ ] 2.1 Wrap all page and data routes (except the public login/register routes and static assets) with the session-identity middleware from the authentication capability
...
- [ ] 2.4 Render login and `/register` pages and handle their POSTs; set the session token in an `HttpOnly`, `Secure`, `SameSite=Lax` cookie on login; clear it on logout
- [ ] 2.5 Issue and validate a CSRF token on all state-changing POSTs (login, register, scan start, cancel, custom request, logout)
...
- [ ] 6.1 Render the dashboard with summary statistics fragments (totals, findings-by-severity, active scans) using the persistence summary-counts query restricted to the requesting user's sessions (unrestricted for admin)
