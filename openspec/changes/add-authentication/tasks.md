...
- [ ] 2.1 Add a `sessions` table (opaque token, user_id, created_at, expires_at, last_seen_at)
...
- [ ] 2.5 Treat expired sessions as invalid and implement removal of expired sessions (lazy or swept)
- [ ] 2.6 Implement `logout(token)` that invalidates the session immediately

## 3. Ownership and visibility
- [ ] 3.1 Stamp the creating user's id onto each scan session at creation as an immutable owner attribute
