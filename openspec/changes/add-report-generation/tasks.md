...
## 7. Tests (local only — no network)
- [ ] 7.1 Build an in-memory fixture session with findings spanning several severities
- [ ] 7.2 Assert the Markdown report contains target, severity breakdown counts, and each finding's type/severity/endpoint/remediation
- [ ] 7.3 Assert evidence appears with include-evidence on and is absent with it off
- [ ] 7.4 Assert the CSV has a header plus exactly one row per reportable finding, correctly escaped
- [ ] 7.5 Parse the JSON export and assert session count, per-session metadata, and finding fields
- [ ] 7.6 Assert the HackerOne export leads with the most-severe finding and errors on a session with no reportable findings
- [ ] 7.7 Assert benign/non-finding results are excluded from every format
