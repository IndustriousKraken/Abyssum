...
- [x] Accept two or more named identities on the CLI (repeatable `--identity`),
...
- [x] Run the selected scanners once per identity, attaching that identity's
      credential via `Orchestrator::with_credential`; a single identity behaves as an
      ordinary scan.
- [x] Ensure every identity's requests route through `ScanContext::send` so the
      pacing floor and User-Agent rotation apply to each pass.
- [x] Compare per-identity results and emit an access-control finding when a resource
...
- [x] Persist and render the differential findings like any other findings, naming
      the identities involved in the evidence.
- [x] Test: a surface where identity-B can read identity-A's resource yields a
