...
- [ ] Add a `subdomain_recon` scanner implementing `BaseScanner`, registered via
      `register_builtins`; `validate_target` requires a bare host (no path).
- [ ] Discover candidate subdomains from passive certificate-transparency / passive-DNS
...
- [ ] Probe each candidate through `ScanContext::send` to determine liveness; report each
      live subdomain as an informational finding recording the host.
- [ ] Detect subdomain takeover by matching the probe response against known
      unclaimed-service fingerprints; emit a high-severity vulnerable finding naming the
      subdomain and the suspected service.
- [ ] Deduplicate candidates and cap the number probed to a sane bound; log when the cap
      truncates results rather than silently dropping them.
- [ ] Test (no real network): given a stubbed passive source and stubbed HTTP responses,
