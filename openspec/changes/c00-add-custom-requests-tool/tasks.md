...
- [ ] 1.1 Add a `custom_request` module in `abyssum-core` with a request spec (url, method, headers, optional body, optional content type, follow-redirects flag, timeout)
...
- [ ] 2.2 Capture status code, response headers, body, round-trip elapsed time, and the redirect chain
...
## 4. Output rendering
- [ ] 4.1 Render a human-readable form: request line, status, timing, redirects, response headers, a truncated body preview, and the signals
