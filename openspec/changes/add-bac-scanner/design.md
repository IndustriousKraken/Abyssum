...
- **Wordlist:** obtained from the seeded reference-data store (see `add-seed-data`), looked
  up by scanner id. The curated admin/sensitive-path list ships in
  `assets/seed/wordlists/paths.txt` (with `paths_short.txt` as a fast profile) and is seeded
  into the database on first run: `/admin`, `/api/admin`, `/api/users`, `/dashboard`,
