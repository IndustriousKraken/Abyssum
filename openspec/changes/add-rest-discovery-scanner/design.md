...
- **Wordlist:** obtained from the seeded reference-data store (see `add-seed-data`), looked
  up by scanner id. The curated list ships in `assets/seed/wordlists/` (`endpoints.txt`,
  `api_bases.txt`) and is seeded into the database on first run. No user-uploaded wordlists
