...
Implement the tool in `abyssum-core` so both surfaces share one code path (the CLI in #12
and the web UI in #14 each render the same result). The tool takes a request specification,
...
- **CLI argument shape (wired in #12):** mirrors the v1 flags — `-X/--method`,
