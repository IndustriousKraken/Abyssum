...
## Purpose
TBD - created by archiving change a00-bootstrap-rust-workspace. Update Purpose after archive.
## Requirements
...
### Requirement: Database Location Configuration
The configuration SHALL include a database file location with a built-in default, so that
later persistence can resolve where to store data without defining its own configuration. The
location SHALL be overridable through the file and environment like any other setting.

#### Scenario: Default database location is present
- **WHEN** the system loads configuration with no overrides
- **THEN** the configuration SHALL provide a non-empty default database file location

#### Scenario: Database location is overridable
- **GIVEN** a configuration file or environment override specifying a different database location
- **WHEN** the system loads configuration
- **THEN** the overriding value SHALL take effect
